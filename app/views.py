...
def session_update_view(request, pk):
    ...
    return render(request, 'app/session_update.html', {'Shakl': form})
...
def semester_list_view(request):
    ...
    return render(request, 'app/semester_list.html', {"Smestrlar": semesters, })
            ...
            if data == 'True':
                ...
                try:
                    if Semester.objects.get(semester=semester, session=ss):
                        messages.error(request, semester + " uchun " + session.session + " sessiya allaqachon mavjud")
